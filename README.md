...
- Step11 から割り込み処理が必要になるので、先に実装しておく。
- intr_linux.go
    - C言語の SIGRTMIN は Go からは参照できないようなので、syscall.SIGUSR1 で代用する。
        - 利用できるIRQ 番号は SIGUSR1 と SIGUSR2 の2つに限定されてしまうが、今回は1つしか使用しないはずなのでこれで進める。
        - IRQ 番号に対応する構造体のメンバや関数の引数はシグナルで代用する。
    - シグナル受信処理を行うためにスレッドは作らず Go ルーチンを利用する。そのため、 pthread_xxx()系の関数に対応する処理はほぼ不要になる。
    - Go ルーチンの起動直後の同期や終了指示にはチャネルを利用する。そのため、SIGHUP は使用しない。
- ここでランタイムを最新版の Go 1.24 にアップした。

### リファクタリング

- 全体
    - テストコードからどこを参照することになるか分からないため、microps パッケージの関数や構造体メンバ等は公開（先頭大文字）に統一しておく。
- loogback.go
    - このファイルだけ driver パッケージに分割する意味を感じなくなったため、microps パッケージに移す。
